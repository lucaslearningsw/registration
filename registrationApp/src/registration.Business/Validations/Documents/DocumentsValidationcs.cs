...
namespace registration.Business.Validations.Documents
{
    public class CpfValidacao
    {
        ...
        {
            var NumbersCpf = Utils.JustNumbers(cpf);

            if (!LengthValid(NumbersCpf)) return false;
            return !HasRepeatDigits(NumbersCpf) && HasDigitsValid(NumbersCpf);
        }
        ...
        private static bool HasRepeatDigits(string value)
        {
        ...
        {
            var number = valor.Substring(0, LengthCpf - 2);
            ...
            return string.Concat(firstDigit, secondDigit) == valor.Substring(LengthCpf - 2, 2);
        }
    }
